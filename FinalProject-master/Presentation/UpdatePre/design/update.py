            ...
            for i, v in enumerate(values[1:], start=1):
                try:
                    if base is None or v is None or base <= 0 or v <= 0:
                        ...
                        cagr = ((v / base) ** (1 / i) - 1) * 100
                        result.append(round(cagr, 2))
                except Exception as e:
                    print(f"⚠️ Error in CAGR calc at i={i}, v={v}, base={base}: {e}")
                    result.append(None)
            return result
            ...
            if slide.shapes.title and "company overview" in slide.shapes.title.text.strip().lower():
                ...
                for key, value in lines.items():
                    ...
                    if isinstance(value, list):
                        for item in value:
                            para = text_frame.add_paragraph() if not first_line else text_frame.paragraphs[0]
                            para.text = item
                            para.font.size = Pt(16)  # קטן יותר לבולטים
                            para.alignment = PP_ALIGN.LEFT
                        ...
                        para.font.size = Pt(18)  # קטן יותר לכותרות רגילות
                        para.alignment = PP_ALIGN.LEFT
                        para.line_spacing = Pt(28)
                        first_line = False
                return
...
def Create(lines,new_ratios, timeseries_data,years, sentiment, section_data_positive, section_data_negative,
           forecast, recommendation, reasons,output):
    ...
    ''' builder.create_swot_slide("Strengths", {"Strengths": section_data_positive.get("Strengths", [])})
    builder.create_swot_slide("Opportunities", {"Opportunities": section_data_positive.get("Opportunities", [])})
    builder.create_swot_slide("Weaknesses", {"Weaknesses": section_data_negative.get("Weaknesses", [])})
    builder.create_swot_slide("Threats", {"Threats": section_data_negative.get("Threats", [])})
'''
